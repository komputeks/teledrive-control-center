...
import { ArrowRight, Bot, CheckCircle2, Cloud, Compass, FolderKanban, Globe, PlayCircle, RefreshCw, Rocket, ShieldCheck, Sparkles, UploadCloud, Wand2 } from 'lucide-react';
...
export function WorkspaceDashboard() {
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,rgba(125,211,252,0.24),transparent_28%),linear-gradient(180deg,#f8fbff,#eef6ff_38%,#e7f0ff_100%)] text-slate-900 dark:bg-[radial-gradient(circle_at_top,rgba(56,189,248,0.18),transparent_24%),linear-gradient(180deg,#020617,#081225_36%,#0f172a_100%)] dark:text-white">
      <div className="mx-auto flex max-w-7xl flex-col gap-8 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
        <section className="relative overflow-hidden rounded-[36px] border border-white/50 bg-white/80 p-6 shadow-[0_35px_120px_-45px_rgba(15,23,42,0.35)] backdrop-blur-xl dark:border-white/10 dark:bg-white/5 lg:p-8">
          ...
          <div className="relative grid gap-8 xl:grid-cols-[1.15fr_0.85fr]">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-sky-300/50 bg-sky-100/80 px-3 py-1 text-xs font-semibold uppercase tracking-[0.22em] text-sky-700 dark:border-sky-300/20 dark:bg-sky-400/10 dark:text-sky-200">
                <Sparkles className="h-3.5 w-3.5" /> Built for nerds, but finally pleasant
              </div>
              <h1 className="mt-5 max-w-4xl text-4xl font-semibold tracking-tight text-slate-950 dark:text-white sm:text-5xl lg:text-6xl">
                Turn Telegram storage into a clean, friendly SaaS your team can understand in minutes.
              </h1>
              <p className="mt-5 max-w-3xl text-base leading-8 text-slate-600 dark:text-slate-300 sm:text-lg">
                TeleDrive now feels like a polished SaaS product: guided onboarding, simple upload flows, visible progress, clear mental models, and zero scavenger-hunt UX. Pick a workspace, connect a bot, choose a folder, and start moving files.
              </p>
              <div className="mt-6 flex flex-wrap gap-3">
                <button onClick={() => hydrate(selectedBucket)} className="inline-flex items-center gap-2 rounded-2xl bg-slate-950 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-slate-900/15 transition hover:translate-y-[-1px] dark:bg-sky-400 dark:text-slate-950"><RefreshCw className="h-4 w-4" /> Refresh workspace</button>
                <button className="inline-flex items-center gap-2 rounded-2xl border border-slate-300/60 bg-white px-5 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-white/10 dark:bg-white/5 dark:text-white"><Compass className="h-4 w-4" /> Guided setup</button>
              </div>
            </div>
            ...
            <div className="grid gap-4 rounded-[32px] border border-slate-200/60 bg-slate-50/90 p-5 dark:border-white/10 dark:bg-slate-950/40">
              ...
              <div className="h-3 overflow-hidden rounded-full bg-slate-200 dark:bg-white/10">
                <div className="h-full rounded-full bg-gradient-to-r from-sky-400 via-cyan-400 to-emerald-400" style={{ width: `${(onboardingProgress / 4) * 100}%` }} />
              </div>
              ...
              {buckets.map((bucket) => (
                <button key={bucket.id} onClick={() => setSelectedBucket(bucket.id)} className={`w-full rounded-[24px] border p-4 text-left transition ${selectedBucket === bucket.id ? 'border-sky-300 bg-sky-50 shadow-[0_18px_45px_-28px_rgba(56,189,248,0.55)] dark:border-sky-400/30 dark:bg-sky-400/10' : 'border-slate-200/80 bg-white dark:border-white/10 dark:bg-white/5'}`}>
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <div className="font-semibold text-slate-950 dark:text-white">{bucket.name}</div>
                      <div className="mt-1 text-sm text-slate-500 dark:text-slate-400">/{bucket.slug} · {bucket.region}</div>
                    </div>
                    <span className="rounded-full border border-slate-300/80 px-3 py-1 text-xs font-medium uppercase tracking-[0.18em] text-slate-600 dark:border-white/10 dark:text-slate-300">{bucket.visibility}</span>
                  </div>
                </button>
              ))}
            </div>
                ...
                {folders.map((folder) => (
                  <button key={folder.id} onClick={() => setSelectedPath(folder.path)} className={`flex w-full items-center justify-between rounded-2xl border px-4 py-3 text-left transition ${selectedPath === folder.path ? 'border-emerald-300 bg-emerald-50 dark:border-emerald-400/30 dark:bg-emerald-400/10' : 'border-slate-200/80 bg-white dark:border-white/10 dark:bg-white/5'}`}>
                    <div>
                      ...
                      <div className="text-xs text-slate-500 dark:text-slate-400">{folder.path}</div>
                    </div>
                  ...
                  {integrations.map((integration) => (
                    <div key={integration.id} className="rounded-2xl border border-slate-200/80 bg-slate-50/80 p-4 dark:border-white/10 dark:bg-slate-950/40">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <div className="font-semibold text-slate-950 dark:text-white">{integration.workspace_name}</div>
                          <div className="mt-1 text-sm text-slate-500 dark:text-slate-400">@{integration.bot_username} · {integration.target_chat_id}</div>
                        </div>
                        <span className={`rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] ${statusTone[integration.status] || 'border-slate-300 text-slate-600 dark:border-white/10 dark:text-slate-300'}`}>{integration.status}</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="mt-5 grid gap-3">
                  {transfers.map((transfer) => (
                    <article key={transfer.id} className="rounded-2xl border border-slate-200/80 bg-slate-50/80 p-4 dark:border-white/10 dark:bg-slate-950/40">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <div className="font-semibold text-slate-950 dark:text-white">{transfer.file_name}</div>
                          <div className="mt-1 text-sm text-slate-500 dark:text-slate-400">{transfer.source_type.toUpperCase()} → {transfer.destination_path}</div>
                        </div>
                        <span className={`rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] ${statusTone[transfer.status] || 'border-slate-300 text-slate-600 dark:border-white/10 dark:text-slate-300'}`}>{transfer.status}</span>
                      </div>
                      <div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-200 dark:bg-white/10">
                        <div className="h-full rounded-full bg-gradient-to-r from-sky-400 via-cyan-400 to-emerald-400" style={{ width: `${Math.max(8, transfer.progress)}%` }} />
                      </div>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        <button onClick={() => updateTransfer(transfer.id, transfer.status === 'paused' ? 'uploading' : 'paused')} className="inline-flex items-center gap-2 rounded-xl border border-slate-300/80 bg-white px-3 py-2 text-xs font-semibold text-slate-700 dark:border-white/10 dark:bg-white/5 dark:text-white">{transfer.status === 'paused' ? <PlayCircle className="h-3.5 w-3.5" /> : <PlayCircle className="h-3.5 w-3.5" />}{transfer.status === 'paused' ? 'Resume' : 'Pause'}</button>
                        <button onClick={() => updateTransfer(transfer.id, 'queued')} className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-3 py-2 text-xs font-semibold text-white dark:bg-sky-400 dark:text-slate-950"><RefreshCw className="h-3.5 w-3.5" /> Retry</button>
                      </div>
                    </article>
                  ))}
                </div>
        ...
        <section className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-[32px] border border-slate-200/60 bg-white/80 p-6 shadow-[0_20px_80px_-40px_rgba(15,23,42,0.22)] backdrop-blur-xl dark:border-white/10 dark:bg-white/5">
            ...
            <div className="mt-5 space-y-3">
              {loading ? <div className="rounded-2xl border border-slate-200/80 bg-slate-50/80 p-5 text-sm text-slate-500 dark:border-white/10 dark:bg-slate-950/40 dark:text-slate-400">Loading files…</div> : scopedFiles.map((file) => (
                <div key={file.id} className="flex flex-col gap-4 rounded-[26px] border border-slate-200/80 bg-slate-50/80 p-5 dark:border-white/10 dark:bg-slate-950/40 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    <div className="flex items-center gap-3">
                      <div className="rounded-2xl bg-slate-900 p-2 text-white dark:bg-white/10"><CheckCircle2 className="h-4 w-4" /></div>
                      <div>
                        <div className="font-semibold text-slate-950 dark:text-white">{file.name}</div>
                        <div className="mt-1 text-sm text-slate-500 dark:text-slate-400">{file.folder_path} · {file.mime_type}</div>
                      </div>
                    </div>
                  </div>
                  <div className="flex flex-wrap items-center gap-3">
                    <div className="text-sm font-medium text-slate-600 dark:text-slate-300">{formatBytes(file.size_bytes)}</div>
                    <span className={`rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] ${statusTone[file.status] || 'border-slate-300 text-slate-600 dark:border-white/10 dark:text-slate-300'}`}>{file.status}</span>
                    {file.download_slug ? (
                      <a className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-3 py-2 text-xs font-semibold text-white dark:bg-sky-400 dark:text-slate-950" href={`/api/next/download/${file.download_slug}`}>Open download plan <ArrowRight className="h-3.5 w-3.5" /></a>
                    ) : (
                      <button onClick={() => createSignedLink(file.id)} className="inline-flex items-center gap-2 rounded-xl border border-slate-300/80 bg-white px-3 py-2 text-xs font-semibold text-slate-700 dark:border-white/10 dark:bg-white/5 dark:text-white"><ShieldCheck className="h-3.5 w-3.5" /> Create link</button>
                    )}
                  </div>
                </div>
              ))}
          ...
          <div className="rounded-[32px] border border-slate-200/60 bg-white/80 p-6 shadow-[0_20px_80px_-40px_rgba(15,23,42,0.22)] backdrop-blur-xl dark:border-white/10 dark:bg-white/5">
            ...
            <div className="mt-5 grid gap-3">
              {[
                ...
                <div key={title} className="rounded-2xl border border-slate-200/80 bg-slate-50/80 p-4 dark:border-white/10 dark:bg-slate-950/40">
